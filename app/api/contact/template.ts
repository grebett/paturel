...
<mjml>
  <mj-head>
    <mj-title>New Contact - Paturel Notaires</mj-title>
    <mj-preview>New message from ${firstname} ${lastname} - ${subject}</mj-preview>
    <mj-attributes>
      <mj-all font-family="'Figtree', Helvetica, Arial, sans-serif" color="#4A4A4A" />
      <mj-text font-size="16px" line-height="24px" />
      <mj-section padding="0px" />
  ...
  <mj-body background-color="#FFFBFA">
    ...
    <mj-section padding="0 16px">
      <mj-column background-color="#FFFFFF" border-radius="4px" padding="32px 24px" border-top="4px solid #CC978A">
        ...
        <mj-table cellpadding="4px" table-layout="fixed">
          <tr>
            <td style="width:100px; color:#CC978A; font-weight:600; font-size:14px;">Name:</td>
            <td style="color:#4A4A4A; font-weight:600;">${firstname} ${lastname}</td>
          </tr>
          <tr>
            <td style="width:100px; color:#CC978A; font-weight:600; font-size:14px;">Email:</td>
          ...
          <tr>
            <td style="width:100px; color:#CC978A; font-weight:600; font-size:14px;">Phone:</td>
            <td style="color:#4A4A4A;">${phone || "Not provided"}</td>
          </tr>
          <tr>
            <td style="width:100px; color:#CC978A; font-weight:600; font-size:14px;">Subject:</td>
            <td style="color:#4A4A4A; font-weight:600;">${subject}</td>
          </tr>
        </mj-table>
        ...
        <!-- CORRECTION : DIV HTML standard pour background et border-radius -->
        <mj-text padding="0">
          <div style="background-color:#FBF2EE; border-radius:4px; padding:16px; font-style:italic; color:#4A4A4A; line-height:26px;">
            « ${message.replace(/\n/g, "<br />")} »
          </div>
    ...
    <mj-section padding="24px 0 48px 0">
      <mj-column>
        <mj-text align="center" color="#999999" font-size="12px">
          This email was sent automatically from www.paturel-notaires.fr
        </mj-text>
      </mj-column>
    </mj-section>
...
export const mailTextEn = ({ firstname, lastname, email, phone, subject, message }: { firstname: string, lastname: string, email: string, phone: string, subject: string, message: string }) => `
Hello. You have received a new message from the Paturel Notaires website:
...
export const mailSubjectEn = ({ firstname, lastname, subject }: { firstname: string, lastname: string, subject: string }) => `New message from ${firstname} ${lastname} (${subject})`;